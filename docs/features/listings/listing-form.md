...
- **Photos** — the gallery in its order, the cover marked, a control to add more and one to remove or promote each. A photo may be offered before there is a listing to hold it. Until one exists the photo waits, shown beside the stored ones and counted with them, and the trader may take it back before it is ever stored. The moment the listing exists the waiting photos are stored in the order they were offered, so the first one chosen is the cover. A trader who offers a photo and leaves stores nothing. A gallery already holding the marketplace's maximum offers no way to add another rather than taking one and refusing it. The count names how many the gallery holds against the marketplace's maximum, and the tile naming what may be added draws its file types and its limit from what the gallery actually accepts. A marketplace requiring no photos says the gallery is optional rather than telling the trader one is needed.
...
- **Save or publish** — the primary action, the secondary one, what publishing still needs, and, set apart from them, the one control that does not keep the listing: removing it. Removing asks first, names the listing and says its photos go with it, and cannot be undone. A listing that has not been saved has nothing to remove, so it is offered nothing.
...
A trader who asks to save or publish a listing that is not ready is told so out loud as well as on the fields, since pressing something and watching nothing happen says nothing. Nothing is written, and any photos already offered keep waiting.
