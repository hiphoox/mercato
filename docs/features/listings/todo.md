...
9. [ ] `ListingImage` with `listing_id`, `storage_key`, `position`, `is_cover`
10. [ ] Upload through the existing storage port so the local-disk adapter is the default and Tigris stays opt-in — see [ports.md](../../architecture/ports.md)
...
12. [ ] Exactly one cover per listing, enforced at the data layer
13. [ ] Server-side type and size validation on upload
14. [ ] Deleting a listing deletes its stored objects
