defmodule MercatoWeb.Layouts.UserMenuTest do
  ...
  describe "signed in" do
    test "shows the user's name" do
      assert [current_user: @user] |> document() |> LazyHTML.text() =~ "Jane Doe"
    end
    ...
    test "styles sign out as destructive" do
      classes =
        [current_user: @user] |> query("a[href='/sign-out']") |> LazyHTML.attribute("class")

      assert hd(classes) =~ "text-error-text"
    end

    test "marks each row as a menu item" do
      assert [current_user: @user] |> query("[role=menuitem]") |> Enum.count() > 0
    end
  end
  ...
  describe "signed out" do
    ...
    test "does not show an account identity block" do
      refute [current_user: nil] |> document() |> LazyHTML.text() =~ "@"
    end

    test "still renders a trigger, so the menu is reachable" do
      assert [current_user: nil] |> query("#user-menu-trigger") |> Enum.count() == 1
    end
  end
  ...
  describe "display name" do
    test "falls back to the handle when there is no name" do
      ...
      assert [current_user: user] |> document() |> LazyHTML.text() =~ "janedoe"
    end

    test "uses the avatar image when the user has uploaded one" do
      user = %{@user | avatar_url: "/uploads/jane.png"}

      assert [current_user: user] |> query("#user-menu-trigger img") |> LazyHTML.attribute("src") ==
               ["/uploads/jane.png"]
    end
  end
end
